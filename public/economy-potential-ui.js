// Potencial de economia por grupo - detalhe sob demanda
(function(){
  ...
  function renderGroup(){
    ...
    const rows=all.filter(r=>r[2]===selectedGroup&&(Number(r[9])||0)>0&&(!q||String(r[0]).includes(q)||String(r[1]).toUpperCase().includes(q))).sort((a,b)=>(Number(b[9])||0)-(Number(a[9])||0));
    if(!window.ECONOMY_FULL_READY&&all.length<Number(s.opportunities||0)){body.innerHTML='<div class="economy-loading">Carregando a lista completa de itens...</div>';return}
    const total=rows.reduce((a,r)=>a+(Number(r[9])||0),0);
    drawer.querySelector('.economy-drawer-head p').textContent=`${rows.length} itens com potencial • ${br(total)} no filtro atual`;
  ...
  function openGroup(group){selectedGroup=group;drawer.querySelector('h3').textContent=group;drawer.querySelector('#economyDrawerSearch').value='';backdrop.classList.add('open');drawer.classList.add('open');renderGroup()}
  gbody.addEventListener('click',e=>{const tr=e.target.closest('tr[data-group]');if(tr)openGroup(tr.dataset.group)});
  drawer.querySelector('#economyDrawerSearch').addEventListener('input',renderGroup);
  window.addEventListener('economy-full-data-ready',()=>{if(drawer.classList.contains('open'))renderGroup()});
